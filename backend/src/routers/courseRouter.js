...
router.get(
    "/get-all",
    limiters.courseLimiter,
    checkTokenOptional,
    validator(courseValidations.getAllCoursesQuerySchema, "query"),
    courseController.getAllCourses
)

router.get(
    "/:slug/get-students",
    validator(courseValidations.getCourseStudentsQuerySchema, "query"),
    checkToken,
    checkUserBan,
    checkRoles(["admin", "teacher"]),
    checkSelfs.checkSelfCourseAuthor(true),
    courseController.getCourseStudents
)

router.get(
    "/:slug/get-comments",
    validator(courseValidations.getCourseCommentsQuerySchema, "query"),
    checkToken,
    checkUserBan,
    checkRoles(["admin", "teacher"]),
    checkSelfs.checkSelfCourseAuthor(true),
...
router.post(
    ...
    checkUserBan,
    checkRoles(["admin", "teacher"]),
    validator(courseValidations.createSchema),
...
router.post(
    ...
    checkUserBan,
    checkRoles(["admin", "teacher"]),
    limiters.enrollLimiter,
    checkSelfCourseAuthor(false),
    courseController.registerUserInCourseByTeacher
)
