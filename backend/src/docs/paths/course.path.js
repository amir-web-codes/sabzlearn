module.exports = {
    "/courses/get-all": {
        get: {
            ...
            description: `Public Course-list endpoint with optional bearer authentication. Middleware order: Course limiter -> optional bearer-token validation -> query validation -> controller. When Authorization is omitted, the request is allowed. When it is supplied, the token must be valid. status is special: non-admin callers may request only published; admins may explicitly request draft/published/archived/closed. If status is omitted, the backend returns published Courses even for admins. category matches the selected category plus every descendant category. minPrice/maxPrice filter finalPrice (after discount), not base price. Price-range requests bypass list caching. Returned Courses are full raw lean documents with unpopulated category/tags/instructor. The empty state is [].`,
            security: [
                {},
                {
                    bearerAuth: []
    ...
    "/courses/{slug}/get-students": {
        get: {
            ...
            description: `Management endpoint for active Course enrollments. Middleware order: query validation -> authentication -> ban check -> admin/teacher role check -> checkSelfCourseAuthor(true) -> controller. Admins may access any non-deleted Course. Teachers may access only Courses they instruct. Course status is not restricted by the ownership lookup. Only active enrollments are counted/listed. The service sorts Enrollment rows by createdAt or lastAccessedAt, but the controller then returns only populated user objects (_id/username/email); the enrollment timestamps are not exposed. The empty state is the exact string "no student found", not an empty array. meta.rating is the Course's full { average, count } object.`,
            security: [
                {
                    bearerAuth: []
                }
    ...
    "/courses/{slug}": {
        ...
        patch: {
            ...
            description: `Updates a non-deleted Course. Middleware order: Course limiter -> authentication -> ban check -> admin/teacher role -> checkSelfCourseAuthor(false) -> body validation -> controller. Because adminAllowed=false, being an admin does NOT let a caller edit another instructor's Course. Every body field is optional and {} is valid. A title change generates a fresh unique slug, so frontend state/router links should use data.slug from the response after a successful title edit. category, when supplied, is a category slug and must resolve to a category whose status is not inactive. The current API has no null/empty-category input to clear an existing category. tags is an array of up to five Tag ObjectIds; [] clears all tags.`,
            security: [
                {
                    bearerAuth: []
                }
        ...
        delete: {
            ...
            description: `Soft-deletes a Course and hard-deletes all of its Lesson documents inside the same MongoDB transaction. Authentication, ban check, admin/teacher role, and checkSelfCourseAuthor(true) apply. Therefore the instructor may delete their Course and an admin may delete another instructor's Course. After transaction commit, Course/Lesson caches are invalidated and stored lesson videos are deleted from Cloudinary on a best-effort basis. The Course document itself remains in MongoDB with isDeleted=true/deletedBy/deletedAt. The controller returns only success/message.`,
            security: [
                {
                    bearerAuth: []
                }
    ...
    "/courses/create": {
        post: {
            ...
            description: `Creates a Course owned by the authenticated admin/teacher. Authentication -> ban check -> admin/teacher role -> body validation -> controller. title and description are required. Defaults supplied by Zod are price=0, discountPercentage=0, level=beginner, language=fa, status=draft. The backend generates a unique slug from title. category is an optional active-category slug. tags is an optional array of at most five Tag ObjectIds; every id must exist. The response is the full raw Course document with unpopulated references.`,
            security: [
                {
                    bearerAuth: []
                }
    ...
    "/courses/{slug}/enroll/{id}": {
        post: {
            ...
            description: `Instructor-management endpoint that upserts an Enrollment. id is validated as a MongoDB ObjectId before authentication. Then authentication -> ban check -> admin/teacher role -> enrollment limiter -> checkSelfCourseAuthor(false) -> controller. Since adminAllowed=false, even an admin must be the Course instructor to use this endpoint. The target user must exist with isDeleted=false and isBanned=false. A missing/cancelled enrollment is created/reactivated as active; an already-active enrollment is updated as active and lastAccessedAt is refreshed. The endpoint always returns 201 on success and synchronizes Course.studentsCount to the number of active enrollments.`,
            security: [
                {
                    bearerAuth: []
                }
    ...
    "/courses/{slug}/thumbnail": {
        patch: {
            ...
            description: `Uploads/replaces a Course thumbnail. Only the Course instructor may use this route; checkSelfCourseAuthor(false) gives no admin override. multipart field name must be thumbnail. Allowed MIME types: image/jpeg, image/png, image/webp. Maximum size: 2 MiB. The new image is uploaded and saved before the previous Cloudinary image is deleted. Cache invalidation happens after save. The full raw Course document is returned.`,
            security: [
                {
                    bearerAuth: []
                }
        ...
        delete: {
            ...
            description: `Removes the custom thumbnail and restores { url: "/images/default-thumbnail.png", publicId: null }. Only the Course instructor may use this route; admin has no ownership override. If thumbnail.publicId is already null, the endpoint returns 409.`,
            security: [
                {
                    bearerAuth: []
                }
    ...
    "/courses/{slug}/cover-video": {
        patch: {
            ...
            description: `Uploads/replaces the Course cover video. Only the Course instructor may use this route; admin has no ownership override. multipart field name must be coverVideo. Current video middleware accepts video/mp4 only, maximum 100 MiB. The full raw Course document is returned after save.`,
            security: [
                {
                    bearerAuth: []
                }
        ...
        delete: {
            ...
            description: `Deletes the stored Course cover-video reference and resets it to { url: null, publicId: null }. Only the Course instructor may use this route; admin has no ownership override. If coverVideoURL.publicId is already null, the endpoint returns 409.`,
            security: [
                {
                    bearerAuth: []
                }
